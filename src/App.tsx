...
export default function App() {
  ...
  const handleSendMessage = async (e?: React.FormEvent, starterText?: string) => {
    ...
    try {
      // 1. Sync user message to backend
      await axios.post('/api/chat', newUserMsg);

      // 2. Prepare System Prompt
      const systemPrompt = `You are Aanya, a warm, empathetic, evidence-based maternal health companion. 
Acknowledge emotions before info, use simple language, and keep replies under 180 words.

Empathy Guidelines:
- Use phrases like "I hear how challenging this is," "It's completely understandable to feel wiped out," or "You're doing an amazing job navigating these changes."
- For sleep issues: Acknowledge the frustration of elusive rest. Suggest small comforts like pillow positioning or calming routines.
- For physical discomfort: Validate that aches and pressures are real and taxing. Remind her that resting is a productive part of growing a baby.

Mandatory Safety:
ALWAYS remind users to consult their OB-GYN for diagnosis or red-flag symptoms like:
...
- LMP: ${profile?.last_period_date || 'Not set'}
`;

      // 3. Call Gemini for response
      const response = await genAI.models.generateContent({
        ...
        contents: messages.concat(newUserMsg).slice(-10).map(m => ({
          role: m.role === 'assistant' ? 'model' : 'user',
          parts: [{ text: m.content }]
        }))
      });

      const replyContent = response.text || "I'm here for you. How are you feeling today?";
      ...
      // 4. Sync assistant message
      await axios.post('/api/chat', assistantMsg);
...
User message: "${text}"`;
        ...
        const extractionText = extractionRes.text || "[]";
        const match = extractionText.match(/\[.*\]/s);
        if (match) {
          const extracted = JSON.parse(match[0]);
          if (extracted.length > 0) {
            const logsRes = await axios.post('/api/logs', extracted.map((item: any) => ({
              ...item,
              note: "Auto-logged from chat"
            })));
            
            setLogs(prev => [...(Array.isArray(logsRes.data) ? logsRes.data : [logsRes.data]), ...prev]);
            toast.success(`Logged from chat: ${extracted.map((e: any) => e.type).join(', ')}`);
          }
        }
      } catch (exErr) {
      ...
      toast.error(`Aanya is momentarily unavailable: ${errorMessage}`);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-[1600px] mx-auto w-full px-6 py-8 grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8">
        ...
        <div className="relative h-[600px] lg:h-[calc(100vh-140px)] lg:sticky lg:top-24">
            ...
            <div className="p-4 border-b border-border-custom flex items-center justify-between bg-surface/30 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] uppercase text-ink-muted">Powered by Claude Sonnet 4.5</p>
                </div>
              </div>
